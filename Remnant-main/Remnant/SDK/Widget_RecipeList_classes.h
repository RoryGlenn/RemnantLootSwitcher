...
public:
	...
	unsigned char                                      UnknownData_TTB7[0x4];                                     // 0x0324(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	void OnAttemptedCraft__DelegateSignature(const struct FRecipeInfo& Recipe);
	void OnClearRecipes__DelegateSignature();
	void OnRecipeClick__DelegateSignature(const struct FRecipeInfo& Recipe);
	void OnSelectRecipe__DelegateSignature(const struct FRecipeInfo& Recipe, class UWidget_Recipe_C* RecipeWidget);
};
