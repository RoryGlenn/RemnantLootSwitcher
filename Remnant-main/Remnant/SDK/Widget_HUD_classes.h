...
// 0x02BC (FullSize[0x04FC] - InheritedSize[0x0240])
...
public:
	...
	unsigned char                                      UnknownData_3OSQ[0x2];                                     // 0x04BA(0x0002) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	unsigned char                                      UnknownData_VC0W[0x3];                                     // 0x04F5(0x0003) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	void GetUserSetting(const struct FName& UserSetting, class UUserSetting** Result);
	void UpdateHUDSettings(const struct FName& UserSetting, class UWidget* Widget, TEnumAsByte<EHudSettings> Type, float SizeMin, float SizeMax, bool FixedAspectRatio_);
	...
	void OnHudEvent(const struct FName& Event, class UObject* Context);
	void Construct();
	...
	void ShowSummonPopup(const struct FName& SummonerName, float Duration);
	...
	void EnteringZoneAnimation(const struct FText& ZoneName);
