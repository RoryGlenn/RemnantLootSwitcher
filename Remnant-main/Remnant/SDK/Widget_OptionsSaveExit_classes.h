...
// 0x0071 (FullSize[0x02BA] - InheritedSize[0x0249])
class UWidget_OptionsSaveExit_C : public UWidget_UserWidget_C
{
public:
	unsigned char                                      UnknownData_ZZVA[0x7];                                     // 0x0249(0x0007) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	void CheckExitState(const struct FName& Action, bool* ShowingWarning);
	void ShowExitWarning(const struct FName& Reason, class AUIActor** UIActor);
	...
	void OnExitKickBan(const struct FName& Result);
	void Destruct();
	void OnDialogResult(const struct FName& Result);
	void UpdateMatchmakingSettings();
	void ExitToMenuWarningResult(const struct FName& Result);
	void ExitGameWarningResult(const struct FName& Result);
